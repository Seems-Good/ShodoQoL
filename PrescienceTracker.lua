...
--   A second one-shot expiry timer fires at expirationTime to guarantee the
--     red transition even if UNIT_AURA is dropped in combat.
...
-- Each fires exactly once at expirationTime to force the red transition
-- in case UNIT_AURA is throttled / dropped during combat.
...
row1.slotFS:SetText("|cffaaaaaa P1|r")
row2.slotFS:SetText("|cffaaaaaa P2|r")
...
--   • expiry fires at  expirationTime             → rescan + red
-- The expiry timer is the combat safety net: if UNIT_AURA is throttled
-- or dropped at buff expiry, this guarantees the orange→red transition.
------------------------------------------------------------------------
local function ScheduleBuffTimers(slotIdx, expirationTime)
    ...
    -- Fire +0.1 s after expirationTime so the server has had time to remove
    -- the aura from the API.  Even if ScanUnit somehow still sees it, we
    -- force hasBuff = false when GetTime() is past the known expiry —
    -- the buff cannot legitimately still be active at that point.
    if expiryIn > 0 then
...
sSubFS:SetText("|cff888888Live Prescience state for P1/P2 — purely event-driven, zero CPU at rest|r")
...
previewLabelFS:SetPoint("TOPLEFT", sDiv1, "BOTTOMLEFT", 0, -14)
...
infoFS:SetText(
    ...
    .. "|cffff9900o Orange|r  Expiring (≤ " .. WARN_SEC .. "s)    "
    .. "|cffff3333x Red|r  Missing\n"
    .. "|cff555555x Grey|r  Target not in group"
)
